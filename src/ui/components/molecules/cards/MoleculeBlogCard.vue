...
<template>
  <div class="card">
    <img class="rounded-lg" :src="props.imgSrc" alt="image" />
    <span class="text-base text-gray text-left">{{ props.date }}</span>
    <h3 class="card__title">{{ title }}</h3>
    <p class="card__text">{{ text }}</p>

    <AtomLink v-if="isLinkExists" :link="linkInfo!.to">{{ linkInfo!.label }}</AtomLink>

    <div v-if="isTagsExists" class="flex w-full flex-wrap gap-3">
...
.card {
  @apply shadow-card-3 max-w-[635px] flex flex-col gap-y-5 p-6 rounded items-start;
}
.card__icon {
